...
// ينقل أي صورة مخزّنة كـ base64 إلى مفتاح منفصل img:<id> ويستبدلها برابط /api/img؟id=
async function offloadImages(data){
let changed=false;
for(const c of (data.menu||[])){
for(const it of (c.items||[])){
if(typeof it.img==="string" && it.img.indexOf("data:")===0){
const id=String(it.id);
try{ await cmd(["SET","img:"+id,it.img]); it.imgV=Date.now(); it.img="/api/img?id="+id; changed=true; }catch(_){}
}
}
}
return changed;
...
// انقل أي صور جديدة رفعها الأدمن إلى مفاتيح منفصلة قبل الحفظ
await offloadImages(next);
