// /api/i18n-fix — أداة لمرة واحدة: تنظيف أسماء المنيو الإنجليزية (أدمن فقط)
// GET ?key=ADMIN_PASSWORD            → تقرير تجريبي (Dry Run) بدون أي حفظ
// GET ?key=ADMIN_PASSWORD&apply=1    → تنفيذ فعلي وحفظ المنيو
// المنطق لكل صنف:
//   1) لو الوصف مكتوب إنجليزي بالغلط → ينقله لخانة nameEn ويفضّي الوصف
//   2) لو nameEn لسه فاضية → يجيب الاسم الإنجليزي من قاموس qonsole
//   3) تصحيح ترجمات معروفة غلط (Impregnation → Tashreeb ... إلخ)
// وكذلك يضيف الاسم الإنجليزي للأقسام (catEn).
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    const rep = { mode: apply ? "APPLIED ✅ تم الحفظ" : "DRY-RUN 👀 معاينة فقط — أضف &apply=1 للتنفيذ", cats: [], fromDesc: [], fromQonsole: [], fixedExisting: [], alreadyOk: 0, stillMissing: [] };

    for (const c of (data.menu || [])) {
      if (!c.catEn) {
        const m = CMAP[norm(c.cat)];
        if (m) { c.catEn = m; rep.cats.push({ ar: c.cat, en: m }); }
      }
    ...
    if (apply) await cmd(["SET", "menu_data", JSON.stringify(data)]);
    rep.summary = {
