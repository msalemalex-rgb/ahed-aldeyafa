...
// حد أقصى للطلبات: عدّاد بنافذة زمنية. لو قاعدة البيانات وقعت، ما نوقفش الزبائن الحقيقيين.
async function rateLimit(bucket, limit, windowSec) {
  const key = "rl:" + bucket;
  try {
    const r = await pipe([["INCR", key], ["EXPIRE", key, String(windowSec)]]);
    const n = Number(r && r[0]) || 0;
    return { ok: n <= limit, count: n, limit, retryAfter: windowSec };
  } catch (_) { return { ok: true, count: 0, limit, degraded: true }; }
}

// IP الزبون من خلف Vercel
function clientIp(req) {
  const h = (req && req.headers) || {};
  const xf = String(h["x-forwarded-for"] || "").split(",")[0].trim();
  return xf || String(h["x-real-ip"] || "").trim() || "unknown";
}

module.exports = { cmd, pipe, newId, rateLimit, clientIp, addOrder, listOrders, setOrderStatus, addReservation, listReservations };
