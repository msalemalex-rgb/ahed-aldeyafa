...
function priceOrder(data, order) {
  ...
  // الخصم المتدرّج: نسبة أقل تحت حدّ معيّن وأعلى فوقه — نفس القاعدة اللي
  // بيحسب بيها المتصفح بالضبط، وإلا رفض السيرفر المبلغ واتقفل الدفع.
  let disc = Number(st.directDiscount) || 0;
  const th = Number(st.discThreshold) || 0;
  if (th > 0 && st.discLow != null && st.discHigh != null) {
    disc = raw >= th ? Number(st.discHigh) : Number(st.discLow);
    if (!isFinite(disc)) disc = Number(st.directDiscount) || 0;
  }
